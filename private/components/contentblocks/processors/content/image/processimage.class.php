...
class ContentBlocksImageProcessor extends modProcessor {
    ...
    public $allowedFileTypes = 'png,gif,jpg,jpeg,svg,webp';
    public $animatedFileTypes = ['gif','webp'];
    ...
    public function cleanFilename($fileName, $fileExtension)
    {
        ...
        if ($this->source instanceof modFileMediaSource || $this->source instanceof \MODX\Revolution\Sources\modFileMediaSource) {
    ...
    /**
     * Takes an Imagine image object and a file extension string.
     * @param ImageInterface $img
     * @param string $fileExtension
     * @return bool
     */
    public function isAnimated(ImageInterface $img, string $fileExtension): bool
    {
        return count($img->layers()) > 1 && in_array($fileExtension, $this->animatedFileTypes);
    }
}
