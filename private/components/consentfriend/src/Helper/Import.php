...
use ConsentfriendServiceContexts;
...
class Import extends ConsentFriend
{
        ...
        foreach ($data as $row) {
            ...
            if ($type == 'update' || $type == 'new') {
                ...
                if ($this->getOption('useContexts')) {
                    $contexts = ($row['contexts'] != '') ? explode(',', $row['contexts']) : [];
                    if (!$this->handleContexts($service, $contexts)) {
                        throw new Exception($this->modx->lexicon('consentfriend.service_err_importing_contexts', ['contexts' => implode(',', $row['contexts'])]));
                    }
                }
            }
        }
     ...
     * @return string
     */
    public function prepareValue(xPDOObject $setting, $value)
    {
     ...
     * @return bool
     */
    private function handlePurposes(xPDOObject $service, array $purposeKeys)
    {
        $purposeIds = [];
     ...
     * @return bool
     */
    private function handleContexts(xPDOObject $service, array $contextKeys)
    {
        ...
        foreach ($existingServiceContexts as $existingServiceContext) {
            ...
            if (!in_array($contextKey, $contextKeys)) {
                // Remove not assigned context keys
