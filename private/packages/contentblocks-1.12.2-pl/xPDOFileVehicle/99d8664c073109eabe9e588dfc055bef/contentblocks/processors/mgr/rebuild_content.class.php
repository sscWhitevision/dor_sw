...
class ContentBlocksRebuildProcessor extends modProcessor {
    ...
    public function process()
    {
        ...
        if ($this->contextKey !== 'all') {
            $this->total = $this->modx->getCount('modResource', ['context_key' => $this->contextKey]);
        }
        else {
        ...
        // Close sessions to prevent session locking from freezing the console with standard php sessions [S21919/F1991]
        session_write_close();
        ...
        if ($this->contextKey !== 'all') {
            $resources = $this->modx->getIterator('modResource', ['context_key' => $this->contextKey]);
        }
        else {
