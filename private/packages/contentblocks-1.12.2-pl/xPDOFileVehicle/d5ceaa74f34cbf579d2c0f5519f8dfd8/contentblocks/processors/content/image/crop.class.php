...
class ContentBlocksImageCropProcessor extends ContentBlocksImageProcessor {
    ...
    public function initialize() {
        if (parent::initialize()) {
            // Override default media source as it can change for crops
            if (!empty($this->getProperty('source'))) {
                $sourceId = filter_var($this->getProperty('source'), FILTER_SANITIZE_NUMBER_INT);
                $source = $this->modx->getObject('sources.modMediaSource', [
                    'id' => $sourceId,
                ]);
                if ($source) {
                    $this->source = $source;
    ...
    public function process() {
        ...
        try {
            ...
            $this->isAnimated = $this->isAnimated($img, $extension);
        } catch (Exception $e) {
            ...
            try {
                if ($this->isAnimated) {
                    ...
                    foreach ($img->layers() as $frame) {
                        $frame->resize(new \Imagine\Image\Box($resizeWidth, $resizeHeight));
                    }
                }
                else {
    ...
    {
        return $this->contentBlocks->getImagine();
    }
