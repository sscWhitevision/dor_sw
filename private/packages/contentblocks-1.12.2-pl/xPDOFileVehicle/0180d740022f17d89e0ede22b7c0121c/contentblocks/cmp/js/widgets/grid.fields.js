ContentBlocksComponent.grid.Fields = function(config) {
    ...
    Ext.applyIf(config,{
		...
		columns: this.getColumns(config, exp),
...
Ext.extend(ContentBlocksComponent.grid.Fields,MODx.grid.Grid,{
    getColumns: function(config, exp) {
        var cols = [
            exp,
            {
            ...
            cols.push(
                {
                    ...
                    renderer: function (v, cell, record) {
                        ...
                        return '<span style="color: #aaa;">[[+</span>' + Ext.util.Format.htmlEncode(record.data.parent_properties['key']) + '<span style="color:#aaa;">]]</span>';
                    },
                    width: .15
                },{
                    header: _('contentblocks.repeater.width'),
                    dataIndex: 'width',
                    renderer: function (v, cell, record) {
                        return Ext.util.Format.htmlEncode(record.data.parent_properties['width'])
                    },
                    width: .15
                }
            );
        ...
        cols.push(
            {
                ...
                width: .1,
                renderer: function (v, cell, record) {
                    return Ext.util.Format.htmlEncode(record.data.input_display)
                }
            },{
        ...
        Ext.Msg.confirm(_('contentblocks.export_fields'), _('contentblocks.export_fields.confirm'), function(e) {
            if (e == 'yes') {
                ...
                if (that.config.parent && that.config.parent > 0) {
                    ...
                    var categoryFilter = Ext.getCmp(that.config.id + '-category-filter');
                    if (categoryFilter && categoryFilter.getValue()) {
                        url += '&category=' + categoryFilter.getValue();
                    }
                }
                window.location = url;
            }
