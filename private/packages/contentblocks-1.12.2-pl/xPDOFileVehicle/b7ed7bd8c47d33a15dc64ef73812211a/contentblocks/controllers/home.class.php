...
class ContentblocksHomeManagerController extends ContentblocksManagerController {
    ...
    public function loadCustomCssJs() {
        $this->addJavascript($this->contentblocks->config['assetsUrl'].'cmp/js/widgets/buttons.js');
        ...
        $this->addCss($this->contentblocks->config['assetsUrl'].'dist/mgr.css');
    ...
    public function loadContentBlocksCanvas()
    {
        ...
        $contextKeys = json_encode($this->getContextKeys());

        $this->addHtml(<<<HTML
<script type="text/javascript">
    var ContentBlocksConfig = $config,
        ContentBlocksWrapperCls = "$wrapperCls",
        ContentBlocksContextKeys = $contextKeys;
</script>
HTML
);
        // Assets
     ...
     * @return array
     */
    protected function getContextKeys(): array
    {
        $c = $this->modx->newQuery(modContext::class);
        $c->where([
            'key:!=' => 'mgr'
        ]);
        $c->sortby(modContext::class . '.rank');
