ContentBlocksComponent.grid.Settings = function(config) {
    ...
    Ext.applyIf(config,{
		...
		columns: [{
            ...
            renderer: function (v) {
                return '<span style="color: #aaa;">[[+</span>' + Ext.util.Format.htmlEncode(v) + '<span style="color:#aaa;">]]</span>';
            }
		},{
        ...
        var win = MODx.load({
            ...
            listeners: {
                success: {fn: function(r) {
                    ...
                    rec.set('template', r.template);
                    rec.set('description', r.description);
                    rec.set('type_ahead', r.type_ahead);
                    rec.set('separator', r.separator);
