...
Ext.extend(ContentBlocksComponent.window.Defaults, MODx.Window, {
    ...
    inputChange: function(combo) {
        ...
        Ext.each(fieldNames, function(fieldName) {
            ...
            if (reloadData && clearTargetName === targetFieldName) {
                ...
                clearTarget.store.reload({
                    ...
                    callback : function(records, operation, success) {
                        // At this point we don't know which ids are which type of field without making another request,
                        // so if the processor doesn't return any results. Make sure the current value is wiped.
                        if (records.length < 1) {
                            clearTarget.clearValue();
                        }
                        else {
            ...
            else if (clearTargetName === targetFieldName && items.length > 0) {
                // directly set data
